...
export const formatPostData = (allPosts) => {
    console.warn(allPosts.length);
    let x = sortByDate(allPosts
    .map((post) => ({
    ...
    .filter((post) => {
        if(!post.title || !post.authors) {
            console.warn('filtered out');
            console.warn(post);
        }
        return post.title && post.authors?.length > 0
    })
    .filter((post) => {
        if(!post.data) {
            console.warn('filtered out');
        ...
        if (post.date) {
            if(new Date(post.date.replace('T', ' ')).getTime() > new Date().getTime()) {
                console.warn('filtered out');
                console.warn(post);
            }
